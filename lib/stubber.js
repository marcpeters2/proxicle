...
var handleProxyAction = function (req, res, method, requestUrl) {
  ...
  if (method === 'PUT' && actionUrl === 'setMockResponse') {
    ...
    var _mockResponsesTimesReturnedData = _.mapValues(mockResponsesTimesReturnedMap, (timesReturned) => {
        return { timesReturned: timesReturned }
    })

    if (urlParts.query.detailed) {
      responseBody = _.merge({},
        mockResponses,
        _mockResponsesTimesReturnedData,
        responseDelays
      )
    }
    else {
      responseBody = _mockResponsesTimesReturnedData
    }
    writeResponse(res, 200, JSON.stringify(responseBody, null, 4));
  }
